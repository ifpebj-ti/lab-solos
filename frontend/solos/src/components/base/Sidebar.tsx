        ...
        <div className="bg-primary w-[70px] min-h-screen flex items-center flex-col">
           ...
           <div className="flex flex-col justify-between w-full mt-14 2xl:mt-20 items-center h-full">
                <div>
                    ...
                    <Link to={"/register"} className={`w-11 h-11 ${currentPage === '/register' ? 'bg-background hover:bg-opacity-90' : 'bg-primary hover:bg-green-700'} flex items-center justify-center rounded-md transition-all ease-in-out mb-3`}><PlusIcon fill={currentPage == "/register" ? "#16A34A" : "#fff"}/></Link>
                </div>
                <div>
                    <Link to={"/profile"} className={`w-11 h-11 ${currentPage === '/profile' ? 'bg-background hover:bg-opacity-90' : 'bg-primary hover:bg-green-700'} flex items-center justify-center rounded-md transition-all ease-in-out mb-6`}><ProfileIcon fill={currentPage == "/profile" ? "#16A34A" : "#fff"}/></Link>
                </div>
            </div>
        </div>
    )
