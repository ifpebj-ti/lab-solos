...
function Carousel({ valor, informacoes, imageSrc }: ICarousel) {
  ...
  }, [currentIndex]); // Dependência em currentIndex para reiniciar o intervalo ao clicar
            ...
            >
              <div className="rounded-md flex items-center justify-center w-full h-full border-[1px] border-border hover:bg-cl-table-item transition-all ease-in-out duration-200 gap-x-4 text-clt-2">
                ...
                <div className='w-5/12'>
                  ...
                  <p className='font-inter-regular text-sm text-clt-2 text-wrap'>{informacoes[itemIndex]}</p>
                </div>
              </div>
            </div>
          );
