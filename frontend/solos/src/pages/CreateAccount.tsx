...
const submitCreateAccountSchema = z.object({
  nome: z
    .string()
    .toLowerCase()
    .transform((nome) =>
...
function CreateAccount() {
  ...
  const { register, handleSubmit, formState: { errors }, setValue, watch } = useForm<CreateAccountFormData>({
        ...
        <div className="w-full bg-backgroundMy rounded-b-md p-4 flex items-center flex-col justify-between">
          ...
          <form onSubmit={handleSubmit(postCreateAccount)} className="w-full gap-y-3 flex flex-col mt-2">
            <div className="gap-y-3 gap-x-5 grid grid-cols-2 w-full">
              <InputText label="Nome Completo" type="text" register={register} error={errors.nome?.message} name="nome" />
              <InputText label="Email" type="email" register={register} error={errors.email?.message} name="email" />
              <InputPassword label="Senha" register={register} error={errors.senha?.message} name="senha" />
              <InputPassword label="Confirme sua Senha" register={register} error={errors.repeat?.message} name="repeat" />
              <InputText label="Instituição" type="text" register={register} error={errors.instituicao?.message} name="instituicao" />
              <InputText label="Curso" type="text" register={register} error={errors.curso?.message} name="curso" />
              <InputText label="Telefone" type="text" register={register} error={errors.telefone?.message} name="telefone" />
                            ...
                            {frameworks.map((framework) => (
                              <CommandItem
                                ...
                                onSelect={(currentValue) => {
                                  ...
                                  setOpen(false);
                                  setValue("mentorResponsavel", currentValue === value2 ? "" : currentValue);
                                }}
                              >
                                {framework.label}
                                <Check className={cn("ml-auto h-4 w-4", value2 === framework.value ? "opacity-100" : "opacity-0")} />
                              </CommandItem>
                            ))}
                          </CommandGroup>
                        </CommandList>
            ...
            <button type="submit" className="font-rajdhani-semibold text-white text-base bg-primaryMy h-10 mt-8 w-full rounded-sm">Criar Conta</button>
          </form>
        </div>
      </div>
    </div>
